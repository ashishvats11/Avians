...
export default function Category(){
    const cat_items= category.map(items=>{
        return (
            ...
            <div key={items.id} className=' m-3 lg:flex lg:flex-col justify-center'>
            <img  src={items.img} className={`rounded-2xl hover:scale-100 transition ease-in-out duration-500 h-96 w-96 scale-90 `} />
            </div>
        )
    })
